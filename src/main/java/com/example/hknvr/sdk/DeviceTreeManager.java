...
/**
 * 海康设备树管理器
 * 参考ClientDemo设备树实现，使用SDK已有结构体，不添加新的实体类
 * <p>
 * 使用方法：
 * 1. DeviceTreeManager manager = new DeviceTreeManager();
 * 2. manager.initSDK();
 * 3. DeviceTreeManager.DeviceSession session = manager.login("192.168.1.100", 8000, "admin", "password");
 * 4. if (session != null) {
 *      String tree = manager.getDeviceTreeString(session);
 *      System.out.println(tree);
 *    }
 * 5. manager.logout(session.loginID);
 * 6. manager.cleanupSDK();
 */
public class DeviceTreeManager {

    /**
     * SDK实例
     */
    private final HCNetSDK sdk;

    /**
     * 设备会话缓存：loginID -> DeviceSession
     */
    /**
     * 静态代码块：在类加载时配置动态库路径
     * 确保在任何 SDK 调用之前执行
     */
    static {
        // 设置 JNA 库搜索路径，指向 HCNetSDK.dll 所在目录
        String libPath = "E:\\project\\Myself\\hk-nvr-demo\\src\\main\\resources\\lib\\win64";
        System.setProperty("jna.library.path", libPath);
        System.setProperty("java.library.path", libPath);
    }

    /**
     * 构造方法：通过 JNA 加载 HCNetSDK 动态库
     */
    public DeviceTreeManager() {
        // 通过 JNA 加载 HCNetSDK 动态库
        this.sdk = Native.load("HCNetSDK", HCNetSDK.class);
    }
    ...
    private final Logger logger = LoggerFactory.getLogger(DeviceTreeManager.class.getName());
    ...
    public boolean cleanupSDK() {
        ...
        for (Map.Entry<Integer, DeviceSession> entry : deviceSessions.entrySet()) {
            sdk.NET_DVR_Logout(entry.getKey());
        }
        deviceSessions.clear();
        return sdk.NET_DVR_Cleanup();
    }
    ...
    public DeviceSession login(String ipAddress, int port, String username, String password) {
        ...
        // 调用登录（使用SDK已有的方法签名）
        int loginID = sdk.NET_DVR_Login_V40(struLoginInfo, struDeviceInfo);

        if (loginID < 0) {
            System.err.println("设备登录失败: " + ipAddress + ":" + port +
                    ", 错误码: " + sdk.NET_DVR_GetLastError());
            return null;
        }
    ...
    public boolean logout(int loginID) {
        ...
        if (session != null) {
            boolean result = sdk.NET_DVR_Logout(loginID);
    ...
    private HCNetSDK.NET_DVR_DEVICECFG_V40 getDeviceConfig(int loginID) {
        ...
        boolean result = sdk.NET_DVR_GetDVRConfig(
                loginID,
        ...
        if (result) {
            ...
            System.err.println("获取设备配置失败，错误码: " + sdk.NET_DVR_GetLastError());
            return null;
        }
    }
    ...
    private HCNetSDK.NET_DVR_IPPARACFG_V40 getIPParaConfig(int loginID) {
        ...
        if (result) {
            ...
            System.err.println("获取IP通道配置失败，错误码: " + sdk.NET_DVR_GetLastError());
            return null;
        }
    }
